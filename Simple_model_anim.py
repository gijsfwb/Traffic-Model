...
#Create csv file for animation   
with open('occupancy_list.csv','w',newline='') as file:
    writer = csv.DictWriter(file, fieldnames=["Source", "Target", "Occupancy"])
    writer.writeheader()
    writer.writerows(occupancy_list)
        

#Manually remove the ' characters from the CSV (couldn't find a cleaner way of doing this)
with open('occupancy_list.csv', 'r', newline='') as infile, open('occupancy_list_fixed.csv', 'w', newline='') as outfile:
    reader = csv.reader(infile)
    writer = csv.writer(outfile)

    for row in reader:
        cleaned_row = [''.join(c for c in cell if c != "\'") for cell in row]
